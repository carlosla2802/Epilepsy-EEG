...
def k_fold_cross_validation(dataset, k, model_class, num_epochs, split_type, device):
    ...
    for fold, (train_idx, test_idx) in enumerate(splits, start=1):
        ...
        model = model_class(input_size=128, hidden_size=64, num_classes=2, num_layers=3, bidirectional=False, dropout_rate=0.02349592724420463).to(device)

        criterion = torch.nn.CrossEntropyLoss()
        optimizer = torch.optim.Adam(model.parameters(), lr=0.0019136826388925345)
...
def train_and_evaluate_all_models(data_dir):
    ...
    key = f"{model.__name__}_{split}_final_model"
    print(f"\nTraining and evaluating {key}")

    # Validación cruzada en el conjunto de entrenamiento y validación
    avg_val_metrics, std_val_metrics = k_fold_cross_validation(train_val_dataset, k, model, num_epochs, split, device)

    # Reentrenar final_model con todo el conjunto de entrenamiento y validación
    final_model = model(input_size=128, hidden_size=64, num_classes=2, num_layers=3, bidirectional=False, dropout_rate=0.02349592724420463).to(device)
